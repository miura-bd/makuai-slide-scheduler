...
**ブラウザのウィンドウをすべて閉じてから**、ターミナルで次を実行します。

```sh
./signage.sh
```

登録した内容が、そのままキオスク表示で出ます。Wayland / X11 の判定、SSH から起動したときの環境変数（`WAYLAND_DISPLAY` や `DISPLAY`）の補完、前回の異常終了で出る「復元しますか」バーの抑止は、スクリプト側で済ませます。
...
ブラウザを閉じずに実行すると、Chromium は新しいウィンドウを起動中のプロセスに任せてしまい、キオスクにならず普通のタブが開くだけになります。スクリプトはこれを検知して、閉じるよう案内して止まります。
...
`MAKUAI_PROFILE` を指定すると `signage.sh` は専用のプロファイルで起動します。普段使いのブラウザとサイネージを1台で兼ねる場合に分けられますが、**その場合は登録も `./signage.sh --windowed` から行う必要があります**（保存先が分かれるため）。
